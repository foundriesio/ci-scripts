...
unset DOCKER_HOST
/usr/local/bin/dockerd-entrypoint.sh -D --log-level debug --experimental --tls=false >/archive/dockerd.log 2>&1 &
for i in `seq 12` ; do
	sleep 1
...
for x in $IMAGES ; do
	...
	echo "Pulling the built image to update its manifest in the docker store..."
	run docker pull ${ct_base}:$TAG-$ARCH 
